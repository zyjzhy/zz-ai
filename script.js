...
// 模型名称映射
const modelNameMap = {
    "Qwen/Qwen2.5-7B-Instruct": "免费 Qwen2.5-7B",
    "Qwen/Qwen2.5-Coder-7B-Instruct": "免费 Qwen2.5-Coder",
    "deepseek-ai/DeepSeek-R1-Distill-Qwen-32B": "付费 Qwen-32B",
    "meta-llama/Meta-Llama-3.1-8B-Instruct": "免费 Llama-3.1-8B",
    "deepseek-ai/DeepSeek-R1-Distill-Llama-70B": "付费 Llama-70B",
    "internlm/internlm2_5-7b-chat":"免费 internlm2_5-7b",
    "deepseek-ai/DeepSeek-V3": "付费 DeepSeek-V3",
    "deepseek-ai/DeepSeek-R1": "好贵 DeepSeek-R1",
    "THUDM/glm-4-9b-chat": "免费 glm-4",
    "01-ai/Yi-1.5-9B-Chat-16K": "免费 Yi-1.5-9B",
    "AIDC-AI/Marco-o1":"免费 Marco-o1",
...
function addMessageElement(message) {
    ...
    content = content.replace(/```([\s\S]*?)```/g, '<div class="code-block"><pre><code>$1</code><button class="copy-btn">复制</button></pre></div>');
    content = content.replace(/\$\$([\s\S]*?)\$\$/g, '<mjx-container display="true">$$$1$$</mjx-container>');
    content = content.replace(/\$([\s\S]*?)\$/g, '<mjx-container>$1</mjx-container>');
    ...
    // 为每个代码块添加复制功能
    messageDiv.querySelectorAll('.code-block').forEach((codeBlock) => {
...
function setupCopyToInputButton(messageDiv, content) {
    ...
    if (copyToInputButton) {
        copyToInputButton.addEventListener('click', () => {
            ...
            // 显示成功状态
            const originalText = copyToInputButton.textContent;
            copyToInputButton.textContent = '🆗';
            setTimeout(() => {
                copyToInputButton.textContent = originalText;
            }, 1000); // 1 秒后恢复原样
        });
    }
}
